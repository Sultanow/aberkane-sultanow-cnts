...
int main(int argc, char *argv[]) {
  ...
  */
  ...
  while(tContents>0){
    ...
    while ((n=t[i])==0) i=(i+1)%T_SIZE; // n = first positive number found
    tContents--;t[i]=0;// on supprime l'élément trouvé
    i=(i+1)%T_SIZE; 
    ...
    if (typeA(n)){         // type A
      //printf("typeA: ");
      t[index]=(2*n - 1)/3; tContents++; index=(index+1)%T_SIZE;
      ...
      if ((len(n))<ls+max-1){
        t[index]=v(n); tContents++; index=(index+1)%T_SIZE;
        //printf("=> push: %d\n",t[index-1]);
    } }
    else if (typeB(n)) {  // type B
      //printf("typeB: ");
      if ((len(n))<ls+max-1){
        t[index]=v(n); tContents++; index=(index+1)%T_SIZE;
        //printf("=> push: %d\n",t[index-1]);
    } }
    else if (typeC(n)) {  // type C
      //printf("typeC: ");
      if (((len(n))<ls+max)&&(n>1)){
        t[index]=(4*n - 1)/3; tContents++;
        //printf("=> push: %d ",t[index-1]);
        index=(index+1)%T_SIZE;
      }
      if ((len(n))<ls+max-1){
        t[index]=v(n); tContents++;
        //printf("=> push: %d",t[index-1]);
        index=(index+1)%T_SIZE;
      }
      //printf("\n");
